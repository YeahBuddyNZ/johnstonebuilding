...
| 3 | **The three case studies** | `project-headland.html`, `project-river.html`, `project-showroom.html` | Narrative, specs and details are written to demonstrate the format. *The Timber Showroom* is loosely based on a real job mentioned publicly, but the specifics are invented. |
...
| 7 | **The logo artwork** | header on every page | The header lockup is currently set in type — heavy tracked caps plus an angular red device — matching the logo's structure and colours but not its actual mark. Supply the original as `assets/logo.svg` (or a 2x PNG) and it becomes a one-element swap. |

## Should confirm

- **Service areas** — the site currently claims Hamilton, Cambridge, Te Awamutu, Raglan and the wider Waikato.
...
- **Physical address** — omitted deliberately; the JSON-LD currently carries Hamilton
  and Waikato only. Add the street address there and to `contact.html` if there is a
  public office.
- **The Timber Showroom's location** — kept as Auckland, because that is where the
  publicly-referenced job appears to have been. Every other project sits in the
  Waikato. Correct if wrong.
...
- Hamilton as the base of operations
- The logo: charcoal ground, red mark, light grey wordmark — the site's palette is taken from it
